...
- **통합 로직**: `Air` (스레드 집계는 API용, 출력은 원본 ticket별 유지)와 `Air2`/`Package` (개별 티켓) 방식을 하나의 툴에서 지원.
- **고성능 비동기 처리**: API 속도 제한(Rate Limit)을 준수하며 빠르게 처리.
- **자동 전처리**: 개인정보(여권, 전화번호) 마스킹 및 대화 내용 병합 자동화.
- **SSL 인증서 지원**: 사내망 SSL 인증서 경로 지정 가능.
...
3.  **환경 변수 설정**:
    ...
    - (선택) 사내망 SSL 인증서 경로: `SSL_CERT_PATH=C:\cert\cert_NOL_SSL_2025.crt`
...
### Air
- 필수 컬럼: `ticket_id`, `thread_id`, `inquiry_title`, `inquiry_content` 등 17개 컬럼
- **로직**:
  - 동일한 `thread_id`를 가진 행들을 API 호출용으로만 집계
  - 출력은 원본 행 수 유지 (99행 입력 → 99행 출력)
  - 모든 ticket_id 및 17개 컬럼 유지 + level1/2/3 추가 = 20개 컬럼
